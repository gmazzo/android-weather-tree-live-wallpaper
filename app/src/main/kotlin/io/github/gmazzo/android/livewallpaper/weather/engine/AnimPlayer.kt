...
class AnimPlayer(first: Int, last: Int, duration: Float, loop: Boolean) {
    ...
    var duration: Float = duration
        private set
    private var sTimeElapsed = 0f
    var count: Int = 0
        private set

    init {
        this.numFrames = (this.lastFrame - this.firstFrame) + 1
        this.halfFrameTime = (this.duration / (numFrames.toFloat())) * .5f
        if (this.duration <= 0f) {
            Log.v(
                TAG,
                "AnimPlayer WARNING: Duration shouldn't be zero, setting to .01f"
            )
            this.duration = .01f
        }
        reset()
    ...
    fun reset() {
        this.sTimeElapsed = 0f
        this.count = 0
        this.currentFrame = this.firstFrame
        this.blendFrame = this.firstFrame
        this.blendFrameAmount = 0f
    }
    ...
    fun update(timeDelta: Float) {
        if (!this.isPaused) {
            ...
            if (this.sTimeElapsed < this.duration + this.halfFrameTime) {
                ...
                this.sTimeElapsed =
                    (this.sTimeElapsed - this.duration) + (this.halfFrameTime * 2f)
                count++
            } else {
                this.currentFrame = this.lastFrame
                this.count = 1
